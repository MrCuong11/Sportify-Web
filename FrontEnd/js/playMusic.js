document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!token) {
    ...
    document.querySelector(".playingSong__tool-next").style.pointerEvents =
      "none";
    document.querySelector(".playingSong__tool-next").style.opacity = 0.5;

    document.querySelector(".playingSong__tool-previous").style.pointerEvents =
      "none";
  ...
  function renderLyrics() {
    ...
    if (!lyricsData || lyricsData.length === 0) {
      ...
      noLyricsMessage.textContent = "Không có lời bài hát";
      noLyricsMessage.classList.add("no-lyrics-message"); // Thêm class để tuỳ chỉnh CSS nếu cần
  ...
  expandLyricsBtn.addEventListener("click", () => {
    ...
    if (isVisible) {
      ...
      loadLyrics(currentSongId); // Tải lyrics khi mở
    }
    //  console.log("lỗi")
  ...
  async function loadNonVietnameseSongs() {
    try {
      ...
      // 2. Tìm ID Việt Nam
      const vietnamCountry = countryList.find(
        (country) => country.code === "viet-nam"
      );
      ...
      const vietnameseArtistNames = vietnameseArtistList.map(
        (artist) => artist.name
      );
